...
public class ImageProcessor {
	...
	public ImageProcessor(String sourcePath) {
		...
		displayImg = MUtils.fitBufferedImage(displayImg, new Dimension(CONST.CANVAS_W, CONST.CANVAS_H));
		setupWidthHeight();
		displayImg = MUtils.toGrayscale(displayImg);
		displayImg = MUtils.ensureARGB(displayImg);
		processingImg = MUtils.deepCopy(displayImg);
		ovalizeDisplayImg();
		processingImg = MUtils.ensureRGB(processingImg);
	...
	private void ovalizeDisplayImg() {
		...
		for(int y = 0; y < ylimit; y++) {
			...
			try { displayImg.setRGB(x, y, MUtils.changeOpacity(displayImg.getRGB(x, y), fo)); }
			catch(ArrayIndexOutOfBoundsException e) { 
				System.out.println("case 1"); 
				}
			try { displayImg.setRGB(newX, y, MUtils.changeOpacity(displayImg.getRGB(newX, y), fo)); }
			catch(ArrayIndexOutOfBoundsException e) { 
				System.out.println("case 2"); 
				}
			try { displayImg.setRGB(x, newY, MUtils.changeOpacity(displayImg.getRGB(x, newY), fo)); }
			catch(ArrayIndexOutOfBoundsException e) { 
				System.out.println("case 3"); 
				}
			try { displayImg.setRGB(newX, newY, MUtils.changeOpacity(displayImg.getRGB(newX, newY), fo)); }
			catch(ArrayIndexOutOfBoundsException e) { 
